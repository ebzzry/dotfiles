...
darwin_test && YOUTUBE=yt-dlp_macos
...
function nman {
  ...
  =man $(barf out-path $item)/share/man/man${section}/${item}.${section}.gz 2> /dev/null
}
...
function ee {
  ...
  if [[ $# -eq 0 ]]; then
    if [[ -f "$PWD/.ee" ]]; then
      ...
      e --eval "(burly-open-bookmark \"Burly: $(basename $PWD)\")"
    fi
  else
    e --eval "(burly-open-bookmark \"Burly: $1\")"
  fi
}
...
function emacs@ {
  ...
  ln -s "~/hejmo/ktp/emacs/fkd/.emacs" ~/.emacs
  ln -s "~/hejmo/dat/emacs" ~/.emacs.d
}
...
function spacemacs@ {
  ...
  ln -s "~/hejmo/ktp/spacemacs/fkd/.spacemacs" ~/.spacemacs
  ln -s "~/hejmo/dat/spacemacs" ~/.emacs.d
}
...
function rmf {
 rm -rf ~/.cache/common-lisp/lw-8.0.1-macosx-arm64/Users/ebzzry/hejmo/fkd/lispo/{veda,vera,vega,marie,pierre,valmiz}
}

function rmf! {
  rm -rf ~/.cache/common-lisp/lw-8.0.1-macosx-arm64/Users/ebzzry/hejmo/fkd/lispo
}
...
function km {
  local dir="$HOME/hejmo/ktp/xmodmap"
...
funs=(
  ...
  am "=alsamixer"
  tea "tee -a"
  pl "za ~/hejmo/dok/provlegado/proofreading.pdf"
  ...
  pstree! "s =pstree"
  pelo! "pelo 1.1.1.1"
  pelo@ "pelo! | tee -a $HOME/hejmo/dat/pelo/pelo.log"
  mtr! "s =mtr 8.8.8.8"
  ...
  rails-nix! "+ drails-nix barf shell"
  rails! "rails-nix!"
  rails-nix+ "D=\$PWD drails-nix barf impure-shell"

  # xmonad
  dxmonad "d ~/.xmonad"

  # rakido
  dracket 'cd $(barf out-path racket)'
  pkg "raco pkg"
  frog "raco frog"
  ...
  # haskelo
  ghci! "stack ghci"
  ghci@ "d \$HOME/hejmo/fkd/haskelo/nix-shell barf shell --run ghci"
  ...
  # aliaj
  delpa "d \$HOME/hejmo/ktp/emacs/var/elpa"
  ...
  dnixpkgs "dktpnixpkgs d nixpkgs"
  doverlays "dnixpkgs d overlays"

  dz "d $HOME/hejmo"
  ddotfiles "d $HOME/hejmo/ktp/dotfiles"
  dttt "d $HOME/hejmo/fkd/ttt/ebzzry.com"
  ettt "dttt e ."
  ...
  dt "d ~/Desktop"
  dl "d ~/Downloads"
  dtemplates "d $HOME/hejmo/fkd/templates/"
  dqb "d $HOME/.qutebrowser"
  dqbl "d $HOME/.local/share/qutebrowser"
  dbin "d $HOME/bin"
  dnyxt "d $HOME/ktp/nyxt"
