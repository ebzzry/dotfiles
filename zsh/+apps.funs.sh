...
function pk {
  for name in ${@}; do
    for pid in $(q $name | awk '{print $1}'); do
      kill $pid
    done
  done
}
...
function git {
  ...
  if [[ $# -eq 0 ]]; then
    ...
    case "${op}" in
      ...
      (zc) "${git}" log -G "${@}" --source --all --pretty=format:"%H" ;;
      (zch) "${self}" zc | head -n 1 | c ;;
      (zct) "${self}" zc | tail -n 1 | c ;;
      (zd) "${git}" log -G "${@}" --source --all --pretty=format:"%ad" ;;
      ...
      # edit author and committer names
      (ca) "${self}" fbc "if [[ \"\${GIT_AUTHOR_NAME}\" == \"$1\" && \"\${GIT_AUTHOR_EMAIL}\" == \"$2\" ]]; then
...
funs=(
  ...
  # rubeno
  drails-nix "d ~/src/nikso/rails-nix"
  ...
  # haskelo
  ghci! "stack ghci"
  ghci@ "d ${HOME}/src/haskelo/nix-shell oof shell --run ghci"
