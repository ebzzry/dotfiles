...
function h { which $@ }
function wh { h "$@" > /dev/null 2>&1 }
function hr { fullpath "$(h $@)" }
function hs { for i ($@) { if wh $i; then print $i; break; fi } }
...
funs=(
  ...
  vh "v ${HOME}/.zhistory"
  zh "v ${HOME}/.zhistory"
  zh! "xzless ${HOME}/Developer/dat/zsh/historio/latest"
  #q "exit"
