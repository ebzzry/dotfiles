...
if linux_pc_test || darwin_test; then
  ...
  if [[ "$TERM" == "dumb" ]]; then
    ...
    #git
    PS1='%F{cyan}%B%n%b%f%B%F{yellow} %f%b%B%F{$hostcolor}%m%f%b%F{yellow}%B %b%f%F{green}%B%~%b%f${vcs_info_msg_0_}%b%D{%n}%F{green}%B>%b%f%F{white} '

    # standard
    # PS1='%F{cyan}%B%n%b%f%B%F{magenta} %f%b%B%F{$hostcolor}%m%f%b %F{green}%B%~%b%f %b%D{%n}%F{green}%B>%b%f%F{white} '

    # nix-darwin
    #PS1='%F{green}%B>%b%f%F{white} '
  fi
else
  PS1='%n %m %0d %D{%n}> '
fi
