...
(cl-defmacro def-cmd (name (&rest args) docstring &rest body)
  `(def ,name (,@args)
     ,docstring
     (interactive)
     ,@body))
...
(def-cmd swap-windows (direction)
...
(def-cmd my-other-window (&optional arg)
...
(def-cmd my-workspace-kill-forward ()
...
(def-cmd my-workspace-kill-backward ()
...
(def-cmd my-garbage-collect-workspaces ()
...
(def-cmd my-load-all-workspaces ()
...
(def-cmd my-save-all-workspaces ()
...
(def-cmd my-scroll-all-windows-to-bottom ()
  "Move all windows to bottom."
  (my-map-windows
   #'(lambda ()
       (my-goto-bottom))))

(def-cmd my-expand-all-windows ()
...
(def-cmd my-collapse-all-windows ()
...
(def-cmd my-rename-all-workspaces ()
...
(def-cmd my-garbage-collect-buffers ()
...
(def-cmd my-org-capture ()
  "Do org-capture"
  (org-capture nil "a"))

(def-cmd my-open-a-org ()
  "Open a.org file"
  (with-new-workspace-home
   (find-file (ef "~/org/a.org"))))

(def-cmd my-open-scratch-buffer ()
  "Open the scratch buffer."
  (with-new-workspace-cwd
   (ci 'doom/open-scratch-buffer)))

(def-cmd my-comment-yank ()
...
(def-cmd my-yank-definition ()
  "Copy the definition to the kill ring and comment it out."
  (ci 'mark-defun)
  (ci 'my-comment-yank))

(def-cmd my-yank-expression ()
  "Copy the expression to the kill ring and comment it out."
  (ci 'mark-sexp)
  (ci 'my-comment-yank))

(def-cmd my-insert-tab ()
  "Insert a tab character."
  (insert "	"))

(def-cmd my-flush-lines ()
  "Delete empty lines"
  (flush-lines "^$"))

(def-cmd my-jump-item ()
...
(def-cmd my-dired ()
  "Open dired from the current directory"
  (dired default-directory))

(def-cmd my-org-roam-capture ()
  "Run org-roam-capture in a new workspace."
  (with-new-workspace-cwd
   (ci 'org-roam-capture)))

(def-cmd my-start-of-line ()
  "Go to the almost start of line. Used with Org."
  (beginning-of-line)
  (forward-char 2))

(def-cmd my-end-of-line ()
  "Go to the almost end of line. Used with Org."
  (end-of-line))

(def-cmd my-mark-org-line ()
...
(def-cmd my-open-lisp-scratch-file ()
  "Open the lisp scratch file."
  (with-new-workspace-home
   (find-file "~/Desktop/t.lisp")))

(def-cmd my-delete-line ()
  "Delete the current line."
  (ci 'mark-line)
  (ci 'delete-region)
  (forward-line 1))

(def-cmd my-kill-line ()
  "Kill the current line."
  (ci 'mark-line)
  (ci 'kill-region)
  (forward-line 1))

(def-cmd my-find-file-new-workspace ()
...
(def-cmd my-toggle-zoom-window ()
  "Toggle the zoom state of a window."
  (let ((register-name (read (cc ":" persp-last-persp-name))))
    (if (register-present-p register-name)
        (progn
